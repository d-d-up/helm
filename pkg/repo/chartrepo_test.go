...
func TestFindChartInRepoURL(t *testing.T) {
	...
	chartURL, err := FindChartInRepoURL(srv.URL, "nginx", "", "", "", "", false, getter.All(&cli.EnvSettings{}))
	if err != nil {
		t.Fatalf("%v", err)
	}
	...
	chartURL, err = FindChartInRepoURL(srv.URL, "nginx", "0.1.0", "", "", "", false, getter.All(&cli.EnvSettings{}))
	if err != nil {
		t.Errorf("%s", err)
	}
...
func TestErrorFindChartInRepoURL(t *testing.T) {
	...
	if _, err := FindChartInRepoURL("http://someserver/something", "nginx", "", "", "", "", false, g); err == nil {
	...
	if _, err = FindChartInRepoURL(srv.URL, "nginx1", "", "", "", "", false, g); err == nil {
	...
	if _, err = FindChartInRepoURL(srv.URL, "nginx1", "0.1.0", "", "", "", false, g); err == nil {
		t.Errorf("Expected error for chart not found, but did not get any errors")
	} else if err.Error() != `chart "nginx1" version "0.1.0" not found in `+srv.URL+` repository` {
		t.Errorf("Expected error for chart not found, but got a different error (%v)", err)
	}

	if _, err = FindChartInRepoURL(srv.URL, "chartWithNoURL", "", "", "", "", false, g); err == nil {
